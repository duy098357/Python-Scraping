"""
Product Catalogue Updater - CSV Output Version
Matches products from Excel to CSV, updates prices, manages catalogue
Fast version - no image scraping, CSV output
"""
...
class ProductUpdater:
    ...
    def process(self):
        ...
        logger.info("=" * 70)
        logger.info("Starting Product Catalogue Update Process (CSV Output)")
        logger.info("=" * 70)
        ...
        for idx, excel_product in self.excel_df.iterrows():
            ...
            if csv_idx is not None:
                ...
                updated_product = self.prepare_product_row(excel_product, self.csv_df.iloc[csv_idx])
